...
type EventRow = readonly [string,string,string,string,string,string,number,number,number];
...
export default function App() {
  ...
  return <div className="app">
    ...
    <main>
      ...
      {tab==='chat'&&<Chat/>}{tab==='moderation'&&role==='teacher'&&<ModerationPanel me={me} onChanged={loadData}/>} {tab==='class'&&<ClassPanel me={me}/>} {tab==='invites'&&<InvitesPanel me={me}/>}   {role==='teacher'&&tab==='events'&&<TeacherPanel me={me} onCreated={loadData}/>} 
      {tab==='profile'&&<Profile me={me} bought={bought} buy={n=>setBought(v=>v.includes(n)?v:[...v,n])}/>}
    </main>
    <nav>{([['home','⌂','Главная'],['search','⌕','Люди'],['events','✦','Ивенты'],['chat','◌','Чаты'],['profile','◉','Профиль'],['class','🏫','Класс'],['invites','✉','Приглашения'],...(role==='teacher'?[['moderation','✓','Заявки']]:[])] as const).map(x=><button className={tab===x[0]?'on':''} onClick={()=>setTab(x[0] as Tab)} key={x[0]}><span>{x[1]}</span><small>{x[2]}</small></button>)}</nav>
    {selectedEvent&&<EventDetails e={selectedEvent} joined={joined.includes(String(selectedEvent[0]))} toggle={toggleJoin} close={()=>setSelectedEvent(null)}/>} {selected&&<div className="modal"><div><Profile me={selected} bought={[]} buy={()=>{}} compact/><Button stretched onClick={()=>setSelected(null)}>Закрыть</Button></div></div>}
  </div>;
}
...
function Event({e,joined,toggle}:{e:readonly (string|number)[];joined:boolean;toggle:(id:string)=>void}){return <article className="event"><div className="cover">{e[5]}</div><div><small>{e[2]} · {e[3]}</small><h3>{e[1]}</h3><p>{e[4]}</p><footer><span>⚡ {e[6]} XP</span><span>🪙 {e[7]}</span><span>👥 {e[8]}</span><Button size="small" onClick={()=>toggle(String(e[0]))}>{joined?'Участвую':'Участвовать'}</Button><button className="event-open" onClick={()=>window.dispatchEvent(new CustomEvent('edugame:event',{detail:e}))}>Подробнее</button></footer></div></article>}
function EventDetails({e,joined,toggle,close}:{e:any;joined:boolean;toggle:(id:string)=>void;close:()=>void}){
 const [participants,setParticipants]=useState<any[]>([]); const [teams,setTeams]=useState<any[]>([]); const [teamName,setTeamName]=useState(''); const [message,setMessage]=useState('');
 useEffect(()=>{load();},[]);
 async function load(){if(!supabase)return; const {data:p}=await supabase.from('event_participants').select('status,profiles(id,full_name,avatar_url,xp)').eq('event_id',String(e[0])).eq('status','joined'); setParticipants(p||[]); const {data:t}=await supabase.from('event_teams').select('id,name,classes(name,grade)').eq('event_id',String(e[0])); setTeams(t||[]);}
 async function createTeam(){if(!supabase||!teamName.trim())return; const {error}=await supabase.from('event_teams').insert({event_id:String(e[0]),name:teamName.trim()}); if(error)setMessage(error.message); else {setTeamName('');setMessage('Команда создана.');load();}}

 return <div className="modal event-modal"><div><button className="close-x" onClick={close}>×</button><div className="event-hero"><span>{e[5]}</span><small>{e[2]} · {e[3]}</small><h1>{e[1]}</h1></div><div className="event-detail-grid"><div><b>⚡ {e[6]}</b><small>XP</small></div><div><b>🪙 {e[7]}</b><small>монет</small></div><div><b>👥 {participants.length||e[8]}</b><small>участников</small></div></div><h3>О мероприятии</h3><p>Участвуй, собирай команду и получай награды за завершение.</p><div className="event-info"><span>📍 Школьная площадка</span><span>📅 {e[3]}</span><span>🏆 Награды выдаются после завершения</span></div><Button stretched onClick={async()=>{toggle(String(e[0]));setTimeout(load,200)}}>{joined?'Вы участвуете':'Участвовать'}</Button><h3>Участники</h3><div className="event-participants">{participants.map((p:any)=><div key={p.profiles.id}><Avatar.Container size={34} form="squircle"><Avatar.Image src={p.profiles.avatar_url||'https://i.pravatar.cc/80?img=12'}/></Avatar.Container><span>{p.profiles.full_name}</span></div>)}</div><h3>Команды</h3>{teams.map((t:any)=><div className="team-row" key={t.id}><b>⚔️ {t.name}</b><small>{t.classes?.grade||t.classes?.name||'Общая команда'}</small></div>)}<div className="team-create"><input value={teamName} onChange={x=>setTeamName(x.target.value)} placeholder="Название команды"/><Button size="small" onClick={createTeam}>Создать</Button></div>{message&&<div className="form-message">{message}</div>}<Button stretched onClick={close}>Закрыть</Button></div></div>
}
function Search({q,setQ,results,open}:{q:string;setQ:(s:string)=>void;results:User[];open:(u:User)=>void}){return <><Typography.Headline>Поиск людей</Typography.Headline><p className="sub">Имя, фамилия, класс или интерес.</p><input className="search-input" placeholder="Имя, фамилия, #Python, 8Б…" value={q} onChange={(e)=>setQ(e.target.value)}/><div className="chips">{['#Python','#Игры','#Фото','#Dota 2'].map(x=><button onClick={()=>setQ(x.slice(1))} key={x}>{x}</button>)}</div>{results.map(u=><button className="result" key={u.id} onClick={()=>open(u)}><Avatar.Container size={54} form="squircle"><Avatar.Image src={u.avatar}/></Avatar.Container><div><b>{u.name}</b><small>{u.className}</small><span>{u.tags.map(t=>'#'+t).join(' ')||'Профиль EDU.GAME'}</span></div><i>→</i></button>)}</>}
...
function InvitesPanel({me}:{me:User}){
 const [items,setItems]=useState<any[]>([]); const [busy,setBusy]=useState<string|null>(null); const [message,setMessage]=useState('');
 useEffect(()=>{load();},[]);
 async function load(){if(!supabase)return; const {data}=await supabase.from('event_classes').select('event_id,class_id,invitation_status,events!inner(id,title,description,event_type,xp_reward,coin_reward,status),classes!inner(id,name,grade,moderator_id)').eq('invitation_status','pending'); setItems(data||[]);}
 async function decide(eventId:string,classId:string,status:'accepted'|'declined'){
   if(!supabase)return; setBusy(eventId+classId); await supabase.from('event_classes').update({invitation_status:status}).eq('event_id',eventId).eq('class_id',classId); setMessage(status==='accepted'?'Класс принял приглашение.':'Класс отказался от приглашения.'); await load(); setBusy(null);
 }
 return <section><Typography.Headline>Приглашения</Typography.Headline><p className="sub">Межклассовые события, на которые приглашён ваш класс.</p>{items.length===0&&<div className="empty">Новых приглашений нет.</div>}{items.map(x=><article className="proposal" key={x.event_id+x.class_id}><small>{x.events.event_type} · {x.classes.grade||x.classes.name}</small><h3>{x.events.title}</h3><p>{x.events.description||'Без описания'}</p><div className="proposal-meta">⚡ {x.events.xp_reward} XP · 🪙 {x.events.coin_reward}</div><div className="proposal-actions"><Button size="small" onClick={()=>decide(x.event_id,x.class_id,'accepted')} disabled={busy===x.event_id+x.class_id}>Принять</Button><Button size="small" onClick={()=>decide(x.event_id,x.class_id,'declined')} disabled={busy===x.event_id+x.class_id}>Отказаться</Button></div></article>)}{message&&<div className="form-message">{message}</div>}</section>
}
